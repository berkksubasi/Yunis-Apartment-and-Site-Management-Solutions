...
export const AidatPaymentScreen = () => {
  ...
  useEffect(() => {
    const fetchPayments = async () => {
      try {
        ...
        const response = await fetch(`https://yunis-api.vercel.app/api/residents/payments/${residentId}`, {
          method: 'GET',
          headers: {
  ...
  const handlePay = async (id: string, amount: number) => {
    try {
      ...
      const response = await fetch('https://yunis-api.vercel.app/api/residents/payAidat', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
